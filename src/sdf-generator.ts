...
export class SdfGenerator {
   ...
   */
  generateAll(changes: StringKeyObject[], templateEntities: StringKeyObject[]) {
  ...
  generateSDFRow(
    ...
    for (const key in templateEntity) {
  ...
  generateSDFCell(
    ...
    const entityFieldName =
      entityName + Config.SDFGeneration.EntityNameDelimiter + key;
    ...
    if (
      ...
      return ExtNameGenerator.generateAndCache(entityName, templateEntity[key]);
    }
