...
import {
  StringKeyObject,
  StringKeyArrayOfObjects,
  StringKeyObjectOfObjects,
} from './shared-types';
  ...
  generateNewSDFForSelectedCampaigns(
    ...
    sheetData.forEach(row => {
      ...
      const campaignsSDF = sdfGenerator.sdfGetFromSheetOrDownload(
        ...
        advertiserId,
        reloadCache &&
          !advertisersForWhichWeAlreadyGotSDF.includes(advertiserId)
      );
    ...
    SpreadsheetApp.getUi().showModalDialog(htmlOutput, '🤗  SDF was generated');
  },
  ...
  saveToSpreadsheet(entities: { [key: string]: StringKeyObject[] }) {
    ...
    try {
      const newSpreadsheet = SpreadsheetApp.create(
        Config.SDFGeneration.NewSpreadsheetTitle
      );
