...
export class SheetCache {
   ...
   */
  constructor(private cacheSheet: GoogleAppsScript.Spreadsheet.Sheet) {}
  ...
  lookup(
    ...
    if (!data || !data.length) {
      console.log(
        `lookup: Empty cache in sheet "${this.cacheSheet.getName()}"`
      );
      return [];
    }
    ...
    if (index < 0 || index > data[0].length - 1) {
      throw Error(
        `Invalid index "${index}" (allowed index range 0..${
          data[0].length - 1
        })`
      );
    }
    ...
    const result = returnOnlyOne
      ? [data.find(searchFunc) as Array<CacheValue>]
      : data.filter(searchFunc);
  ...
  findAndUpdateOneElement(
    ...
    if (!data || !data.length) {
      console.log(
        `findAndUpdateOneElement: Empty cache in sheet "${this.cacheSheet.getName()}"`
      );
      return [];
    }
